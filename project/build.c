 ...
 *  2003 Modified by Zeus Gómez to insert user code
...
void minix_open(const char *name) {
    ...
    if ((fd = open(name, O_RDONLY, 0)) < 0) die("Unable to open `%s': %m", name);
    if (read(fd, buf, MINIX_HEADER_LEN) != MINIX_HEADER_LEN) die("%s: Unable to read header", name);
    if (memcmp(buf, hdr, sizeof(hdr)) || lb[5]) die("%s: Non-Minix header", name);
    if (lb[3]) die("%s: Illegal data segment", name);
    if (lb[4]) die("%s: Illegal bss segment", name);
    if (lb[7]) die("%s: Illegal symbol table", name);
}
...
int main(int argc, char **argv) {
    ...
    if (buf[510] != 0x55 || buf[511] != 0xaa) die("Boot block hasn't got boot flag (0xAA55)");
    buf[508] = minor_root;
    buf[509] = major_root;
    if (write(1, buf, 512) != 512) die("Write call failed");
    close(fd);

    if ((fd = open(argv[2], O_RDONLY, 0)) < 0) /* Copy the system itself */
        die("Unable to open `%s': %m", argv[2]);
    if (fstat(fd, &sb)) die("Unable to stat `%s': %m", argv[2]);
    ...
    if (lseek(1, 500, SEEK_SET) != 500) die("Output: seek failed");
    if (write(1, buf, 2) != 2) die("Write of image length failed");

    buf[0] = (sys_size & 0xff);
    buf[1] = ((sys_size >> 8) & 0xff);
    buf[2] = ((sys_size >> 16) & 0xff);
    buf[3] = 0;
    if (lseek(1, 512, SEEK_SET) != 512) die("Output: seek failed");
    ...
    buf[3] = 0;
    if (lseek(1, 516, SEEK_SET) != 516) die("Output: seek failed");
    if (write(1, buf, 4) != 4) die("Write of user length failed");
    buf[0] = (0x7E6A & 0xff);
    buf[1] = ((0x7E6A >> 8) & 0xff);
    buf[2] = ((0x7E6A >> 16) & 0xff);
    buf[3] = 0;
    if (lseek(1, 520, SEEK_SET) != 520) die("Output: seek failed");
    if (write(1, buf, 4) != 4) die("Write of user length failed");
