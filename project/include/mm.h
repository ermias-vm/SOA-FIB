...
/** Frame is available for allocation */
#define FREE_FRAME 0

/** Frame is currently in use */
#define USED_FRAME 1
 ...
 * Allocates frames for user code and data pages.
 *
 ...
 *
 * Loads the CR3 register with a new page directory address,
 * effectively switching the memory context and flushing the TLB.
 *
 ...
 *
 * Sets up the GDT with kernel and user code/data segments,
 * configures the TSS base address, and loads the GDTR register.
 */
void setGdt(void);
 ...
 * transitions. Configures segment selectors, stack pointers, and
 * initial register values, then loads the TR register.
 */
void setTSS(void);
