...
struct task_struct {
    ...
    int quantum;                          /**< Current process quantum */
    ...
    unsigned long kbd_saved_ctx[SW_AND_HW_CONTEXT_SIZE]; /**< Saved full context (SW + HW) before
                                        handler (11 SW + 5 HW registers) */
};
...
/** Pointer to current running task (optimization) */
extern struct task_struct *current_task;

/** Queue for free (unused) task structures */
extern struct list_head freequeue;

/** Queue for ready-to-run processes */
 ...
 *
 * This function initializes the free queue, ready queue, and blocked queue
 ...
 * executing process by examining the stack pointer.
 *
 ...
 * round-robin scheduling algorithm. Decrements the current quantum.
 */
